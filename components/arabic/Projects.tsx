...
import { PROJECTS } from '../../constantsAr';

export const Projects: React.FC = () => {
  return (
    <section className="py-24 bg-dark-bg" id="projects" dir="rtl">
      <div className="container mx-auto px-6">
        <div className="mb-16">
          <h2 className="text-3xl md:text-4xl font-display font-bold text-white mb-4">أعمال مختارة</h2>
            ...
            <div key={project.id} className="group relative rounded-2xl overflow-hidden bg-dark-card border border-dark-border hover:border-brand-500/50 transition-all duration-300 flex flex-col h-full">
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                <div className="flex items-center space-x-4 pt-4 border-t border-dark-border">
                  {project.demoUrl && (
                    <a href={project.demoUrl} className="flex items-center text-sm text-slate-300 hover:text-white transition-colors ml-4">
                      <ExternalLink size={16} className="ml-1" /> عرض حي
                    </a>
                  )}
                  {project.repoUrl && (
                    <a href={project.repoUrl} className="flex items-center text-sm text-slate-300 hover:text-white transition-colors">
                      <Github size={16} className="ml-1" /> الكود
                    </a>
                  )}
                </div>
              </div>
