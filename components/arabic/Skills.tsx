...
import { SKILLS_DATA } from '../../constantsAr';
import { Cpu, Globe, Server, Database } from 'lucide-react';

export const Skills: React.FC = () => {
  return (
    <section className="py-24 bg-dark-bg relative" id="skills" dir="rtl">
      <div className="container mx-auto px-6">
        <div className="mb-16">
          <h2 className="text-3xl md:text-4xl font-display font-bold text-white mb-4">الترسانة التقنية</h2>
          <div className="h-1 w-20 bg-brand-500 rounded"></div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          {/* Chart */}
          <div className="h-[400px] w-full bg-dark-card/30 rounded-2xl border border-dark-border backdrop-blur-sm p-4" dir="ltr">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             ...
             <SkillCard 
                        ...
                        <SkillCard 
                         ...
                         title="الذكاء الاصطناعي والعمليات" 
...
const SkillCard: React.FC<{ icon: React.ReactNode; title: string; items: string[] }> = ({ icon, title, items }) => (
  <div className="p-6 rounded-xl bg-dark-card border border-dark-border hover:border-brand-500/50 transition-colors group">
    <div className="flex items-center space-x-3 mb-4">
      <div className="p-2 rounded-lg bg-dark-bg group-hover:bg-brand-500/10 transition-colors ml-3">
        {icon}
      </div>
